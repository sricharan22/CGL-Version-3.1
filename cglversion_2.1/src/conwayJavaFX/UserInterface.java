 ...
 * describes the user interface for the Conway's Game of Life
 * </p>
 * 
 * <p>
 * Copyright: Lynn Robert Carter © 2018-05-06
 * </p>
 * 
 * @author Lynn Robert Carter
 * @co-author Sri Charan 2020501031
 * @version 3.1 29-12-2020 An implementation for UI
 * 
...
public class UserInterface {
	...
	// Attributes used to establish the board and control panel within the window
	// provided to us
	...
	int[][] arr;
	boolean[][] cur;		// Declaring the required variables
	boolean[][] next;

	// The User Interface widgets used to control the user interface and start and
	// stop the simulation
	...
	// The attributes used to inform the user if the file name specified exists or
	// not
	private Label message_FileFound = new Label("");
	private Label message_FileNotFound = new Label("");

	// These attributes are used to tell the user, in detail, about errors in the
	// input data
	...
	// These attributes define the Board used by the simulation and the graphical
	// representation
	// There are two Boards. The previous Board and the new Board. Once the new
	// Board has been
	// displayed, it becomes the previous Board for the generation of the next new
	// Board.
	// private Board oddGameBoard = new Board(); // The Board for odd frames of the
	// animation
	private Pane oddCanvas = new Pane(); // Pane that holds its graphical representation

	// private Board evenGameBoard = new Board(); // The Board for even frames of
	// the animation
	...
	public UserInterface(Pane theRoot) {
		...
		// Establish the attribute that the rest of the system can use to manipulate the
		// GUI
		window = theRoot;
		...
		setupLabelUI(label_FileName, "Arial", 18, ConwayMain.WINDOW_WIDTH - 20, Pos.BASELINE_LEFT, marginWidth,
				controlPanelHeight);

		// Establish the text input widget so the user can enter in the name of the file
		// that holds
		// the data about which cells are alive at the start of the simulation
		setupTextUI(text_FileName, "Arial", 18, ConwayMain.WINDOW_WIDTH / 2, Pos.BASELINE_LEFT, marginWidth,
				controlPanelHeight + 24, true);

		// Establish the link between the text input widget and a routine that checks to
		// see if
		...
		// Establish a GUI button the user presses when the file name have been entered
		// and the
		...
		// Establish the link between the button widget and a routine that loads the
		// data into a
		...
		// Link the start button to the routine that sets up the simulation to run and
		// starts it
		button_Start.setOnAction((event) -> {
			startConway();
		});

		// Establish a GUI button that the user can press once the simulation starts to
		// stop it
		setupButtonUI(button_Stop, "Arial", 18, 50, Pos.BASELINE_LEFT, ConwayMain.WINDOW_WIDTH - 80,
				controlPanelHeight + 24);

		// Link the stop button to the routine that stops the simulation and terminates
		// the program
		// execution
		...
		// The following set up the control panel messages for messages and information
		// about errors
	 ...
	 * read it and run the simulation. If a file is not found, a warning message is
	 * displayed and the button is disabled.
	 */
	void checkFileName() {
		...
		if (str_FileName.length() <= 0) { // this routine is called to see if it is a valid filename.
			...
			try { // this routine tries to open it and establish a scanner.
				...
				// valid
				// for this application (User input errors are GUI issues, not simulation
				// issues.)
	...
	/**********
	 * This method is called when the Load button is pressed. It tries to load the
	 * data onto the board for the simulation.
	 */
...
//					System.out.print("hi");
						...
						cell.relocate(6 * i, 6 * j);
						cell.setFill(Color.BLUE);		// creating new cell for the live cell with different color
						oddCanvas.getChildren().add(cell);	// adding cell to Pane
					}
				}
			}
			window.getChildren().add(oddCanvas); // adding pane object oddcanvas to the scene
//			toggle = true;
		}
	...
	/**********
	 * This method removes the start button, sets up the stop button, and starts the
	 * simulation
	 */
	private void startConway() {
		...
		// Start the simulation by means of an animation Timeline using a keyframe
		// duration of
		// 50 milliseconds
	...
	/**********
	 * This method display the current state of the odd board and terminates the
	 * application
	 */
	...
	public void runSimulation() {
		// Use the toggle to flip back and forth between the current generation and next
		// generation boards.
		if (toggle) {						// toggle case condition for true and false
			window.getChildren().remove(oddCanvas);	// removing the elements present on the scene
			window.getChildren().remove(evenCanvas); // removing the elements present on the scene
			oddCanvas = new Pane(); //creating new pane object
			// Your code goes here...
			Board b = new Board(); // creating board object
			next = b.generateNextGeneration(cur); // generating next generation
...
//					System.out.print("hi");
			...
			if (count == cnt) { // condition to check whether both current and next generations are same or not
				System.exit(0);
			}
			cur = next; // assigning next to current
			toggle = false; // making toggle as false
		} else {
			window.getChildren().remove(oddCanvas);  // removing the elements present on the scene
			window.getChildren().remove(evenCanvas);  // removing the elements present on the scene
			evenCanvas = new Pane();  //creating new pane object
			Board b = new Board();  // creating board object
			next = cur; // assigning current to next
			cur = b.generateNextGeneration(next);  // generating next generation
			for (int i = 0; i < cur.length; i++) {
				for (int j = 0; j < cur.length; j++) {
					if (cur[i][j]) {
						Rectangle cell = new Rectangle(5, 5);
						cell.relocate(6 * i, 6 * j);
						cell.setFill(Color.BLUE);  // creating new cell for the live cell with different color
						evenCanvas.getChildren().add(cell);  // adding cell to Pane
					}
				}
			}
			window.getChildren().add(evenCanvas);  // adding pane object oddcanvas to the scene
			int count = 0;
			int cnt = 0;
			for (int i = 0; i < cur.length; i++) {
				for (int j = 0; j < cur.length; j++) {
			...
			toggle = true;  // making toggle as true
		}
	}
	...
	private boolean fileContentsAreValid() {
		...
		int secondValue = -1;
		int lineNumber = 0;
		ArrayList<ArrayList<Integer>> arr1 = new ArrayList<ArrayList<Integer>>(); //initializing 2d arraylist
		...
		while (scanner_Input.hasNextLine()) {
			...
			int pos = 0;
			ArrayList<Integer> a = new ArrayList<Integer>();  // creating a new arraylist
			// Read in the line and trim the white space before and after the real data
			String inputLine = scanner_Input.nextLine().trim();

			// Construct a scanner to parse each input line (should be two integer
			// values/line)
			scanner_Line = new Scanner(inputLine);
			String[] nextIntegers = inputLine.split(" "); // creating an array of strings
			...
			a.add(Integer.parseInt(nextIntegers[0])); // adding elements to arraylist
			a.add(Integer.parseInt(nextIntegers[1]));
			arr1.add(pos, a); // adding arraylist to 2d arraylist
			pos += 1;
		}

		arr = new int[lineNumber][2]; // initializing the size of 2d array

		for (int j = 0; j < lineNumber; j++) {
			for (int k = 0; k < 2; k++) {
				arr[j][k] = arr1.get(j).get(k); // assigning the values to the 2d array
